<template>
  <base-popup name="popup-fee-nft" class="popup-fee" width="600px" :close="handleClose">
    ...
    <div class="content" v-loading="isLoading">
      ...
      <div class="content-block">
          <p class="content-block__title">{{popup_data.content.transactionAmount.title}}</p>
          <div class="content-block__inputs">
              <el-input :placeholder="popup_data.content.transactionAmount.input.placeholder1" 
              v-model="popup_data.content.transactionAmount.input.value1"
              class="input-small money" :class="{'input-error': !!(error.transactionAmount || '')}"
              @keypress.native="onlyNumber($event)"
              @keyup.native="numberFormat($event, 'transactionAmount')"
              ></el-input>
              <span style="display:block; width: 8px; height: 2px; background-color: #dbdbdb"></span>
              <el-input :placeholder="popup_data.content.transactionAmount.input.placeholder2" 
              v-model="popup_data.content.transactionAmount.input.value2"
              class="input-small money" :class="{'input-error': !!(error.transactionAmount || '')}"
              @keypress.native="onlyNumber($event)"
              @keyup.native="numberFormat($event, 'transactionAmount')"
              ></el-input>
          </div>
          <p class="content-block__alert" v-if="this.error.transactionAmount === 'transactionAmount'">{{popup_data.content.transactionAmount.alert}}</p>
      </div>
      <div class="content-block">
          <p class="content-block__title">{{popup_data.content.feeAmount.title}}</p>
          <div class="content-block__inputs">
              <el-input :placeholder="popup_data.content.feeAmount.input.placeholder1" 
              v-model="popup_data.content.feeAmount.input.value1"
              class="input-small money" :class="{'input-error': !!(error.feeAmount || '')}"
              @keypress.native="onlyNumber($event)"
              @keyup.native="numberFormat($event, 'feeAmount')"
              ></el-input>
              <span style="display:block; width: 8px; height: 2px; background-color: #dbdbdb"></span>
              <el-input :placeholder="popup_data.content.feeAmount.input.placeholder2" 
              v-model="popup_data.content.feeAmount.input.value2"
              class="input-small money" :class="{'input-error': !!(error.feeAmount || '')}"
              @keypress.native="onlyNumber($event)"
              @keyup.native="numberFormat($event, 'feeAmount')"
              ></el-input>
          </div>
          <p class="content-block__alert" v-if="this.error.feeAmount === 'feeAmount'">{{popup_data.content.feeAmount.alert}}</p>
      </div>
      <div class="content-block">
    ...
    <div class="footer" slot="footer">
      <div class="wrap-button">
        <div class="btn-right">
          ...
          :disabled="!!(error.transactionAmount || error.feeAmount || '')"
          >
...
<script lang="ts">
  ...
  @Component
  export default class PopupFeeNft extends Mixins(PopupMixin) {
  ...
  // @Prop({ required: true, type: Object, default: [] }) resetFilter!: Record<string, any>
    ...
    popup_data = {
        ...
        content: {
            ...
            transactionAmount: {
                title: this.$i18n.t('fee-nft.trans-amount'),
                input: {
            ...
            feeAmount: {
                title: this.$i18n.t('fee-nft.fee-amount'),
                input: {
    ...
    handleReset(): void {
        ...
        this.popup_data.content.transactionAmount.input.value1 = ''
        this.popup_data.content.transactionAmount.input.value2 = ''
        this.popup_data.content.feeAmount.input.value1 = ''
        this.popup_data.content.feeAmount.input.value2 = ''
        this.popup_data.content.status.value = 'SUCCESS'
        this.checkValid('transactionAmount')
        this.checkValid('feeAmount')
    }
    handleClose(): void {
      this.setOpenPopup({
