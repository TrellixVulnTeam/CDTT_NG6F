  ...
  <div class="w-100 transaction">
    ...
    <div class="w-100 bo-kyc box-shadow">
      <div class="bg-white wallet-header">
        <div class="be-flex align-center jc-space-between wallet-header__above">
          <div class="wallet-header__above-tabs be-flex">
            <div class="tab-item cursor" v-for="tab in tabs" :key="tab.id" :class="$route.params.type === tab.type ? 'tab-active' : null" @click="handleChangeTab(tab)">
              <span class="text-base">{{ $t(`menu.${tab.title}`) }}</span>
            </div>
          </div>
        </div>
  ...
  export default class TransactionNFT extends Mixins(PopupMixin) {
    ...
    async init(): Promise<void> {
      try {
        ...
        const result = await api.getListTransactionNft(params)
        console.log(result)
    ...
    renderTitleCard(type: 'SALE' | 'BID' | 'OFFER'): string {
      switch (type) {
        case 'SALE':
    ...
    renderIconCard(type: 'SALE' | 'BID' | 'OFFER'): string {
      switch (type) {
        case 'SALE':
