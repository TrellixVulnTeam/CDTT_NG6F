    ...
    <div class="content">
      <main class="content-left">
        <section class="select block">
          ...
          <el-select v-model="popup_data.content.select.value" placeholder="LYNK">
              <el-option
                v-for="item in popup_data.content.select.options"
                :key="item.value"
                :label="item.label"
                :value="item.value">
                <img src="../../../../icons/png/Lynkey.png" alt="" style="width: 20px; height: 20px; float: left; margin-top: 10px; 
                margin-right: 8px;">
        ...
        <section class="upload block">
          ...
          <el-upload
              ...
              <div class="upload-wrapper" v-if="url">
                ...
                <img src="../../../../icons/png/x-circle.png" alt="close preview" 
                class="upload-wrapper__icon" @click.stop="handleClearUpload">
              </div>
              <div class="el-upload__tip" slot="tip"
              v-if="isCreated && (this.url === '')"
              >PNG, GIF, WEBG, MP4 or MP3. Max size 100mb.</div>
          </el-upload>
          <p class="block-alert"
          v-if="isCreated && (this.url === '')"
          >"File" is required</p>
        ...
        <section class="name block">
          ...
          <el-input placeholder='e. g. "Redeemable T-Shirt with logo"' 
          v-model="popup_data.content.name.value"></el-input>
          <p class="block-alert"
          v-if="isCreated && !popup_data.content.name.value">"Name" is not allowed to be empty</p>
        </section>
        ...
        <section class="description block">
          <h2 class="block-title">Description
           ...
           <el-input type="textarea" :placeholder="popup_data.content.description.placeholder" 
           maxlength="200" show-word-limit rows="4" v-model="popup_data.content.description.value" />
        </section>
        ...
        <section class="unlock block">
          ...
          <el-switch class="block-switch" v-model="popup_data.content.switchUnlock.value"
            active-color="#129961"
            inactive-color="#DBDBDB">
          </el-switch>
          <div class="unlock-input" v-if="popup_data.content.switchUnlock.value">
            <el-input  
            v-model="popup_data.content.switchUnlock.input.value"></el-input>
        ...
        <section class="fee-minting block">
          ...
          <el-switch class="block-switch" v-model="popup_data.content.switchFeeMinting.value"
            active-color="#129961"
            inactive-color="#DBDBDB">
          </el-switch>
        </section>
        ...
        <section class="wrapper block">
          <section class="royalties" style="float: left;">
            ...
            <el-input placeholder='e. g. "10%"' 
            v-model="popup_data.content.royalties.value" type="number" min="0" max="50"
            @input="handleInput"></el-input>
          <p class="block-suggestion">Suggested: 0%, 10%, 20%, 30%. Maximum is 50%</p>
          <p class="block-alert"
           v-if="isCreated && !popup_data.content.royalties.value">"Royalties" must be a number</p>
          </section>
          <section class="copies" style="float: left">
            <h2 class="block-title">Number of copies</h2>
            <el-input placeholder='e. g. "10"'
            v-model="popup_data.content.copies.value" type="number" min="0" max="50"></el-input>
          <p class="block-suggestion">Amount of tokens</p>
          <p class="block-alert"
           v-if="isCreated && !popup_data.content.copies.value">"Supply" must be a number</p>
          </section>
          <br style="clear: both;">
        </section>
        ...
        <section class="advance block" v-if="isShowAdvance">
          <section class="properties block">
            <h2 class="block-title">Properties
              <span class="block-title__light">(Optional)</span> 
            </h2>
            <div class="wrap-input">
              <el-input placeholder='e. g. "Size"' 
              v-model="popup_data.content.property.first.leftinput.value"
              style="width: 301px; margin-right: 24px; float: left"></el-input>
              <el-input placeholder='e. g. "M"' 
              v-model="popup_data.content.property.first.rightinput.value"
              style="width: 301px; float: left"></el-input>
              <br style="clear: both;">
            </div>
            <div class="wrap-input" style="margin-top: 16px" v-if="popup_data.content.collection.value == '2'">
              <el-input placeholder='e. g. "Size"'
              v-model="popup_data.content.property.second.leftinput.value"
              style="width: 301px; margin-right: 24px; float: left"></el-input>
              <el-input placeholder='e. g. "M"'
              v-model="popup_data.content.property.second.rightinput.value"
              style="width: 301px; float: left"></el-input>
              <br style="clear: both;">
            </div>
          </section>
          <section class="nft block">
            <h2 class="block-title">
             ...
             <el-input type="textarea" placeholder='Image description in details (do not start with work “image”)' maxlength="200" show-word-limit rows="4" 
        ...
        <section class="notification block" v-if="!checkResult">
         ...
         <el-button class="btn-default-bg btn-600 btn-h-48 is-none-border block"
           style="margin-left: 0px; margin-bottom: 0px;" @click="handleCreateItem"
           >
           Create Item
          </el-button>
      </main>
...
<script lang="ts">
  ...
  export default class PopupForm extends Mixins(PopupMixin) {
    ...
    handleCreateItem():void {
      this.checkResult = this.popup_data.content.upload.value &&
      this.popup_data.content.name.value &&
      this.popup_data.content.royalties.value &&
      this.popup_data.content.copies.value
